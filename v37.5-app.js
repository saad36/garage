...
(() => {
  ...
  function drawLineChart(id,data,unit,money=false){
    ...
    pts.forEach((p,i)=>{
      ...
      const nearRight=p.x>w-95, nearLeft=p.x<pad.l+30;
      let labelX=p.x, align="center";
      if(nearRight){labelX=w-pad.r-4;align="right";}
      ...
      previousLabel={label,x:p.x,y:labelY};
    });
  ...
  function drawMultiLineChart(id,data){
    ...
    actual.forEach((p,i)=>{
      ...
      const nearRight=p.x>w-95;
      let labelX=nearRight?w-pad.r-4:p.x;
    ...
    if(last){
      ...
      ctx.textAlign="right";ctx.fillText(fmtNum(last.value,0),w-pad.r-4,Math.max(12,last.y+(collide?18:-7)));
    }
